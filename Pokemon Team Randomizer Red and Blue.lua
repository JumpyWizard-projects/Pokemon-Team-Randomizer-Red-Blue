...
local function IVcalc(ADIV,SSIV,i)
	local IV = read(ADIV)
	HPIV[i] = 0
	AIV[i] = 0
	DIV[i] = 0
...
local function writeHP(StatHP,StatHP255,StatCurHP,StatCurHP255,i)
  local roundUp = math.ceil
  local roundDown = math.floor
	if HP[i] > 255 then
		write(StatHP255,roundDown(HP[i] / 255))
		write(StatHP,roundUp(HP[i] - read(StatHP255) * 255))
		write(StatCurHP255,roundDown(HP[i] * CurHP[i] / 255))
		write(StatCurHP,roundUp(HP[i] * CurHP[i] - read(StatCurHP255) * 255))
	else
		write(StatHP,roundUp(HP[i]))
		write(StatCurHP,roundUp(HP[i]*CurHP[i]))
    write(StatHP255,0)
    write(StatCurHP255,0)
	end
end


local function Write(StatHP,StatHP255,StatCurHP,StatCurHP255,StatA,StatA255,StatD,StatD255,StatSPEED,StatSPEED255,StatSPECIAL,StatSPECIAL255,i)
  writeHP(StatHP,StatHP255,StatCurHP,StatCurHP255,i)
	if A[i] > 255 then
...
local function Random(StatRandomPokemon,Stattype1,Stattype2)
  local RandomPokemon = Pokemon[math.random(#Pokemon)]
	write(StatRandomPokemon,RandomPokemon)
	write(Stattype1,type1[RandomPokemon])
	write(Stattype2,type2[RandomPokemon])
end



local function Statcalc(StatRandomPokemon,StatHPEV,StatHPEV255,StatAEV,StatAEV255,StatDEV,StatDEV255,StatSPECIALEV,StatSPECIALEV255,StatSPEEDEV,StatSPEEDEV255,StatLevel,i)
  local Pokemon = read(StatRandomPokemon)
  local root = math.sqrt
	HP[i] = (((HPBase[Pokemon] + HPIV[i]) * 2 + (root(read(StatHPEV) + read(StatHPEV255) * 255))/ 4)) * Level[i] / 100 + Level[i] + 10
	A[i] = ((ABase[Pokemon] + AIV[i]) * 2 + (root(read(StatAEV) + read(StatAEV255)* 255))/ 4) * Level[i] / 100 + 5
	D[i] = ((DBase[Pokemon] + DIV[i]) * 2 + (root(read(StatDEV) + read(StatDEV255)* 255))/ 4) * Level[i] / 100 + 5
	SPEED[i] = ((SpeedBase[Pokemon] + SPEEDIV[i])* 2 + (root(read(StatSPEEDEV) + read(StatSPEEDEV255) * 255))/ 4)*Level[i] / 100 + 5
	SPECIAL[i] = ((SpecialBase[Pokemon] + SPECIALIV[i]) * 2 + (root(read(StatSPECIALEV) + read(StatSPECIALEV255)* 255))/ 4) * Level[i] / 100 + 5
end
...
while true do
	...
	elseif Randomize == true and read(0xD057) > 0 then
      ...
      Level[i] = read(0xD18C)
			...
			IVcalc(0xD186,0xD187,i)
			Random(0xD164,0xD170,0xD171)
			Statcalc(0xD164,0xD17D,0xD17C,0xD17F,0xD17E,0xD181,0xD182,0xD196,0xD195,0xD194,0xD193,0xD18C,i)
